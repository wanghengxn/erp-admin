...
<template>
  <div class="sidebar-item">
    <el-sub-menu v-if="settingsStore.app.routeBaseOn !== 'filesystem' && item.path == undefined" :title="generateI18nTitle(item.meta.i18n, item.meta.title)" :index="JSON.stringify(item)">
      <template #title>
        <div class="item">
    ...
    <el-sub-menu v-else :title="generateI18nTitle(item.meta.i18n, item.meta.title)" :index="settingsStore.app.routeBaseOn !== 'filesystem' ? resolveRoutePath(basePath, item.path) : JSON.stringify(item)">
      <template #title>
